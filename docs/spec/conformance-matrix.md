...
| Property | Status | Notes |
|----------|--------|-------|
| `CertificateString` / `CertificateType` | ✅ | POST input (PEM/DER); GET/DELETE; staged per-VM. |
| `Subject` / `Issuer` / `ValidNotBefore` / `ValidNotAfter` / `SerialNumber` | ✅ | parsed for GET when `cryptography` present. |
| `KeyUsage` / `UefiSignatureOwner` | 🟡 | not yet surfaced. |
| private key fields | ➖ | never accepted or emitted (INV-13; rejected pre-storage). |
